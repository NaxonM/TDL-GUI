...
class SettingsDialog(QDialog):
    ...
    def _create_appearance_tab(self):
        ...
        restart_label = QLabel("<i>A restart is required for the theme to fully apply.</i>")
    ...
    def _create_network_tab(self):
        ...
        self.auto_proxy_checkbox.setToolTip("If checked, tdl-gui will attempt to use the proxy configured in your operating system.\nThis is the recommended setting.")
        ...
        self.manual_proxy_input.setToolTip("If automatic detection is off, specify the full proxy URL here.\nExample: socks5://127.0.0.1:1080")
        ...
        self.ntp_server_input.setToolTip("Optional: Specify a custom NTP server to sync time with.\nThis can help with login issues in some networks.")

        timeout_layout = QHBoxLayout()
        self.reconnect_timeout_spinbox = QSpinBox()
        self.reconnect_timeout_spinbox.setRange(0, 999)
        self.reconnect_timeout_spinbox.setToolTip("How long to wait before trying to reconnect if the connection to Telegram is lost.\nSet to 0 for infinite.")
    ...
    def _create_debug_tab(self):
        ...
        self.debug_mode_checkbox.setToolTip("Greatly increases the amount of information in the Log tab and app.log file.\nUseful for troubleshooting.")
        self.timeout_spinbox = QSpinBox()
        self.timeout_spinbox.setRange(0, 86400)
        self.timeout_spinbox.setSuffix(" s")
        self.timeout_spinbox.setToolTip("Maximum time to wait for any tdl command to complete.\nSet to 0 for no timeout (not recommended).")
        ...
        reset_settings_button.setToolTip("Resets all settings in this dialog to their defaults.")
        reset_settings_button.clicked.connect(self._handle_reset_settings)

        reset_data_button = QPushButton("Reset All TDL Data")
        reset_data_button.setStyleSheet("background-color: #D32F2F; border-color: #B71C1C;")
        reset_data_button.setToolTip("Deletes all tdl data, including login sessions and logs.")
    ...
    def _handle_reset_settings(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.load_settings()
            QMessageBox.information(self, "Settings Reset", "UI settings have been reset to their defaults.\nThe theme will update on restart.")
    ...
    def _handle_code_login_click(self):
        ...
        if ok and text.strip():
            ...
            dialog = LoginDialog(self.tdl_path, namespace, self.settings_manager, self.logger, self)
            if dialog.exec():
    ...
    def _handle_qr_login_click(self):
        ...
        if ok and text.strip():
            namespace = text.strip()
            self._apply_settings_from_ui()
            dialog = QRCodeDialog(self.tdl_path, namespace, self.settings_manager, self.logger, self)
            if dialog.exec():
                self._populate_accounts()
                self.account_combo.setCurrentText(namespace)
    ...
    def load_settings(self):
        self.debug_mode_checkbox.setChecked(self.settings_manager.get('debug_mode', False))
        self.storage_path_input.setText(self.settings_manager.get('storage_path', ''))
        self.auto_proxy_checkbox.setChecked(self.settings_manager.get('auto_proxy', True))
        self.manual_proxy_input.setText(self.settings_manager.get('manual_proxy', ''))
        self.storage_driver_combo.setCurrentText(self.settings_manager.get('storage_driver', 'bolt'))
        ...
        self._populate_accounts()
        self.account_combo.setCurrentText(self.settings_manager.get('namespace', 'default'))
    ...
    def _apply_settings_from_ui(self):
        ...
        self.settings_manager.set('storage_driver', self.storage_driver_combo.currentText())
    ...
    def accept(self):
        ...
        if self.original_theme != new_theme:
            QMessageBox.information(self, "Theme Changed", "A restart is required for the new theme to be fully applied.")
